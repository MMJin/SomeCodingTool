...
- (void)initalUI
{
    ...
    CGFloat headerImage_X = (self.headerImageName.length>0)?kScrollViewHeight:0.f;
    totalWidth = headerImage_X;
...
//区分有头图片没图片
    ...
    imagV.frame = CGRectMake(0, 0, headerImage_X, headerImage_X);
    imagV.backgroundColor = [UIColor redColor];
    [self addSubview:imagV];
    
    self.itemScrollView = ({

        UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(headerImage_X, self.scrollViewTop, scrollViewWidth-headerImage_X, kScrollViewHeight)];
